...
<section id="dashboardAdd">
    ...
    <p>{{Response}}</p>
        ...
        <div class="foodname">
          ...
          <input v-model="food.foodName" type="text" name="name" id="name" required>
        </div>

        <div class="desc">
          <label for="desc">Description</label>
          <textarea v-model="food.foodDescription" name="desc" id="desc" cols="30" rows="10" required></textarea>        </div>

        <div class="price">
          <label  for="price">Prix</label>
          <input v-model="food.foodPrice" type="text" name="price" id="price" required>
        </div>

        <div class="foodImage">
          <label  for="foodImage">Image</label>
        <input type="file" id="file" ref="file" v-on:change="handleFileUpload()"/>        </div>
...
export default {
  ...
  methods: {
    ...
    addFood() {
      ...
      axios
        .post("http://localhost:8888/plats", newFood, formData)
        .then(res => {
